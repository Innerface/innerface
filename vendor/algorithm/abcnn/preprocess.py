...
class Word2Vec(object):
    def __init__(self):
        # Load Google's pre-trained Word2Vec model.
        self.model = gensim.models.Word2Vec.load("D:/Code/Word2Vec/chinese.jieba.model")
        self.unknowns = np.random.uniform(-0.01, 0.01, 300).astype("float32")
    ...
    def open_file(self, mode):
        with open("./WikiQA_Corpus/WikiQA-" + mode + ".txt", "r", encoding="utf-8") as f:
            ...
            for line in f:
                ...
                s2 = list(jieba.cut(''.join(s2), cut_all=False, HMM=False))[:40]
                label = int(items[2])
                # if items[2] and isinstance(int(items[2]),int):
                #     label = int(items[2])
                # else:
                #     label = 0
                # if i%100 == 0:
